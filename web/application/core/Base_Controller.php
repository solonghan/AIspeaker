...
class  Base_Controller  extends  CI_Controller  {
	...
	public function operationLog(){
		if(isset($_SERVER['HTTP_X_TOKEN'])){
			$token_info = $this->Jwt_model->verify_token($_SERVER['HTTP_X_TOKEN']);
			$id = $token_info['id'];
			// check user is not delete
			$sql = "SELECT * FROM users WHERE id=?";
			$parmeter = array($id);
			$user = $this->db->query($sql, $parmeter)->row_array();
			if($user['status']=='off'){
				$this->output(True,'token expired.',array('code'=>50012));
			}
			...
			$sql = "INSERT INTO log (user_id,operation,post) VALUES(?,?,?)";
			$parmeter = array($id,$url,$post);
	 ...
	 * @param limit
	 * 
	 * @return array (strat,end)
	 */
	public function pagging($page=1, $limit=10){
