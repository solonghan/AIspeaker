...
class Lunar_model extends CI_Model {
    ...
    function convertSolarMonthToLunar($year,$month) {
        ...
        $month_days_ary = array(0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
    ...
    function getLunarByBetween($year,$between){
        ...
        if($between==0){
            ...
            if ($leapMonth!=0&&$t==$leapMonth+1) {
                $m = ('闰'.$this->getCapitalNum($t- 1,true));
                $m_n = $t - 1;
            }else{
                if ($leapMonth!=0&&$leapMonth+1<$t) {
                    $m = $this->getCapitalNum($t-1, true);
                    $m_n = $t - 1;
                }else{
                    $m = $this->getCapitalNum($t, true);
                    $m_n = $t;
                }
            }
